...
font21 = pygame.font.Font("./Fonts/BIG JOHN.otf", 21)
font70 = pygame.font.Font("./Fonts/BIG JOHN.otf", 70)
white, gray, black = (255,255,255), (77, 77, 77), (25, 25, 25)
blue, red = (0, 113, 188), (165, 2, 39)


def centerprint(variable, x, y, sizeX, sizeY, color = gray, font = pygame.font.Font("./Fonts/BIG JOHN.otf", 36)):
...
def Pong(AI):
	...
	top = pygame.Rect(0, 72, 1280, 1)
	bottom = pygame.Rect(0, 695, 1280, 1)
	comp, user, winscore = 0, 0, 3
	start, count = True, True
	if not AI: PRD, PRU, PLD, PLU = False, False, False, False
	while True:
		mouse = pygame.mouse.get_pos()
		
		# Start the Game
		if start:
			x, y, r = largura/2, altura/2, 10
			x1, y1 = x, y
			PaddleLeft = pygame.Rect(16, 172, 16, 136)
			PaddleRight = pygame.Rect(1248, 172, 16, 136)
			ball = pygame.Rect(x, y, r*2, r*2); ball2 = pygame.Rect(x1, y1, r*2, r*2)
		...
		if AI and xspeed < 0:
		   ...
		   x1 += xspeed1 * 1.25
		   y1 += yspeed1 * 1.25
		   if PaddleLeft.y <= 72 and ball2.y < 72+PaddleLeft.height/2: PaddleLeft.y = 72+2
		   elif PaddleLeft.centery < ball2.centery: PaddleLeft.centery += 2
		   if PaddleLeft.y+PaddleRight.height >= 695-2 and ball2.y > 695-PaddleLeft.height/2: PaddleLeft.y = 695-PaddleLeft.height
		   elif PaddleLeft.centery > ball2.centery: PaddleLeft.centery -= 2
		
		# Mouse Control
		if AI:
			if PaddleRight.y <= 72 and mouse[1]-PaddleRight.height/2 <= 72: PaddleRight.y = 72
			elif (PaddleRight.y+PaddleRight.height) >= 695 and mouse[1]+PaddleRight.height/2 >= 695: PaddleRight.y = 695-PaddleRight.height
			else: PaddleRight.centery = mouse[1]
