...
def Minesweeper(bombs):
	...
	def search(x, y, rows=rows, columns=columns):
		...
		if x >= 0 and x < columns and y >= 0 and y < rows:
			tile = grid[y][x]
			if tile.visible:  # Stop when it reaches an already visited tile
				return
			if tile.bomb:  # Stop if tile is a bomb
				return
	...
	while True: 
		...
		if not win and not lose:  # Stop Couter if Game Ends
		...
		for row in grid:
			...
			for tile in row:
				...
				if mx >= 0 and mx < columns and my >= 0 and my < rows and grid[my][mx].near == 0 and not lose and not grid[my][mx].visible and not win:  # Mouse following
					if grid[my][mx].flag:
				...
				if event.button == 1 and mx >= 0 and mx < columns and my >= 0 and my < rows:
					if not grid[my][mx].flag:  # Lock flagged tile
						...
						if grid[my][mx].bomb:  # Checks Defeat
							lose = True
				if event.button == 3 and mx >= 0 and mx < columns and my >= 0 and my < rows:  # Flag clicked tile
