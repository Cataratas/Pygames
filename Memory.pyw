import pygame, pygame.freetype, random, time
...
def centerprint(variable, x, y, sizeX, sizeY, color, font = pygame.freetype.Font('./Fonts/seguiemj.ttf', 50)):
	color = color[:-1] + (255, )
...
def Memory():
	...
	start, startTime = True, time.time()
	...
	while True:
		...
		for i in range(rows):
			...
			for j in range(columns):
				...
				if grid[i][j].paired == False: win = False
				x += 128
