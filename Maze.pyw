...
def maze():
	class character():
		...
		def show(self):
			global n_keys
			if self.right: draw("./Layout/Player.png", self.x, self.y-5, True)
			else: draw("./Layout/Player.png", self.x, self.y-5, False)
			for i in range(3):
				try:
	...
	class Key():
		...
		def show(self):
			draw("./Layout/Key.png", self.x-3, self.y-5)
	...
	class Exit():
		...
		def show(self, n_keys):
			if n_keys == 3: draw("./Layout/Door.png", self.x-2, self.y-5)
			else: draw("./Layout/Door 0.png", self.x-2, self.y-5)
	...
	def make_maze(Grid, maze_width, maze_height):
		w = (len(Grid[0]) - 1) // 2
		h = (len(Grid) - 1) // 2
	...
	make_maze(Grid, COLUMNS, ROWS)
	
	# Append Maze to List
	for i in range(COLUMNS):
		for j in range(ROWS):
			if Grid[i][j] == 1: Wall.append((29+width*i, 64+width*j))
			elif Grid[i][j] == 0: Path.append(((29+width*i)+8, (64+width*j)+8))
	...
	while True:
		...
		for x in range(q := 29, 1220, width):
			for y in range(64, 659, width):
				if Player.x+(Player.radius*32)+32 > x and Player.x-(Player.radius*32)-32 < x and Player.y+(Player.radius*32)+32 > y and Player.y-(Player.radius*32)-32 < y:
		...
		for x in range(3): draw("./Layout/Key 0.png", 591+35*x, 18)
		for x in range(n_keys): draw("./Layout/Key.png", 591+35*x, 18)
